...
`src/lib/hooks/*` → `src/lib/api.ts` layout, and the `g`-then-key shortcut
...
**Read `src/lib/providers.tsx`** before touching the QueryClient defaults, the
...
from `src/lib/api.ts` — they normalize the server's `ApiErrorBody` envelope
...
`RepoProvider` (`src/lib/repo-context.tsx`) holds the active repo for chrome
