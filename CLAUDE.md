...
# CLAUDE.md — root pointer map

This file is a **thin index**, not documentation. Module-specific conventions
live in `<module>/CLAUDE.md` (lazy-loaded by location); architectural and
...
- `server/CLAUDE.md` — module shape, validation/error/DI conventions, ESM `.js` imports, embeddings gate, `*.it.test.ts` split, boot reaper.
- `client/CLAUDE.md` — TanStack Query as the only fetch path, global error/toast policy, `_components/<Name>/` colocation, vendored UI/contracts, theme + hydration quirks.
- `reviewer-core/CLAUDE.md` — pure engine (no I/O), `src/index.ts` as the public surface, grounding gate, structural INJECTION_GUARD, npm-not-pnpm, build = typecheck.
- `e2e/CLAUDE.md` — JSON-only specs, deterministic locators (no `chat`), hermetic runner, "never `down -v`", read-only seeded fixtures, agent-browser is the framework.
