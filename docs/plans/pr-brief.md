...
### Post-implementation reversal (architecture review, 2026-07-03)

The architecture reviewer OVERTURNED the F4 rejection: the Container-DI convention cited in D3's defense prohibits direct adapter construction but does NOT authorize importing peer modules' service classes — the onion-architecture pitfalls catalog forbids exactly that pattern. The cross-model reviewer (gpt-5-mini) was right. Rework applied: `Container` now exposes lazy `blastService` / `reviewService` getters (mirroring the `repoIntel` facade idiom), and `brief/service.ts` composes peers through the container with zero cross-module service imports. Relatedly, D5's export+import shim was superseded: `resolveLinkedIssue` (+ `LinkedIssueMeta`) moved to `platform/github-utils.ts` as infrastructure-shared utility; `reviews` and `brief` both import it from platform; the `brief/helpers.ts` re-export shim was removed. Lesson recorded for retro: when triaging cross-model findings, verify the cited convention's actual text — precedent-shaped rebuttals can cite the wrong rule.
