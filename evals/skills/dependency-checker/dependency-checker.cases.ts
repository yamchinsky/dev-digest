...
export const cases: SkillCase[] = [
  {
    ...
    grounding: ["```mermaid", "flowchart", "| Package |"],
    practices: [
      ...
      "the report has a size breakdown table with columns for package name, dependency name, installed size, and notes — not just a prose paragraph",
  ...
  {
    name: "correctly classifies large dev-only packages and does not flag legitimate devDeps as critical",
    kind: "quality",
    prompt: `Our repo has some very large packages. Help me understand which ones we need and which we could remove or replace.\n\n${REPO_DATA}`,
    grounding: ["playwright"],
    practices: [
      "playwright (210M) is NOT flagged as a P0 unused dependency — it is a devDependency used in e2e tests, not a runtime dep",
      "playwright's 210M installed size is flagged as a P1 or otherwise noted concern because it is a dev-only package that significantly inflates the e2e environment",
      "next (132M in client) is NOT recommended for removal — it is the core framework for the client package and its size is expected",
      "the answer distinguishes between packages that are large-but-necessary (next, playwright) and packages that are large-and-questionable (date-fns at 22M as a potential lighter-weight alternative exists)",
      "the answer does not recommend removing devDependencies that are actively used by their package's test or build scripts (playwright for e2e, vitest for server/client/reviewer-core)",
    ],
    threshold: 0.6,
    maxTurns: 10,
  },
