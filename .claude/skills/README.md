...
| [impl](impl/SKILL.md) | Workflow | SDD execution orchestrator (`/impl`): runs an Implementation Plan end-to-end — feature branch → implementer waves per DAG (cap 3) → plan-verifier coverage gate (first, via the plan-verifier agent) → capped gap-fix loop → architecture-reviewer + arch-fix loop (≤3, to APPROVE) → spec status flip → `gh pr create`. Specs and plans are authored manually upstream |
